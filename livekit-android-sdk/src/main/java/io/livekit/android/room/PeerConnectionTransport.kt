...
constructor(
    ...
    private suspend fun setMungedSdp(
        sdp: SessionDescription,
        mungedDescription: String,
...
private const val DD_EXTENSION_URI =
        ...
        for ((attribute, fmtp) in fmtps) {
            if (fmtp.payload == codecPayload) {
                ...
                if (!fmtp.config.contains("x-google-start-bitrate")) {
                    newFmtpConfig =
                        "$newFmtpConfig;x-google-start-bitrate=${(trackBr.maxBitrate * startBitrateForSVC).roundToLong()}"
                }
